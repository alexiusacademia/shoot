...
local function showTarget()
  local radius = scWidth*0.1
	target = display.newCircle( radius, scHeight-radius, radius )
...
local function showObstacles()
  ...
  obs1 = display.newRect(target.width+obstacleHeight, scHeight, obstacleHeight, obstacleWidth)
...
function scene:create( event )
  ...
  showObstacles()
  showLevel(level)
  maxStretch = ball.x-target.x-target.width/2-ball.width/2
