...
local function showTarget()
  local radius = scWidth*0.1
	...
	physics.addBody( target, "dynamic", {radius=radius, density=3} )
...
local function showObstacles()
  ...
  obs1 = display.newRect( centerX, target.y+target.height/2+obstacleHeight/2+obstacleWidth/2, obstacleWidth*.75, obstacleHeight )
  obs1:setFillColor(0.5)
  
  obs2 = display.newRect( target.x-target.width, target.y, obstacleHeight, obstacleWidth*1.2 )
  obs2:setFillColor(0.5)
  obs2.rotation = 20
  
  obs3 = display.newRect( target.x+target.width, target.y, obstacleHeight, obstacleWidth*1.2 )
