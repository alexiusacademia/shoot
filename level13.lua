...
local function showTarget()
  local radius = scWidth*0.1
	target = display.newCircle( radius, scHeight-radius-ball.width*1.4, radius )
...
local function showObstacles()
  ...
  obs1 = display.newRect( target.width+obstacleHeight, scHeight-ball.width*1.4, obstacleHeight, obstacleWidth )
  obs1:setFillColor(0.5)
  obs1.anchorY = 1
  
  obs2 = display.newRect( 0, obs1.y-obs1.height, obstacleWidth, obstacleHeight )
...
function scene:create( event )
  ...
  showObstacles()
  showLevel(level)
  maxStretch = getDistance(ball.x, ball.y, target.x, target.y) - ball.width/2 - target.width/2-obs1.width
